...
function FaqHeader({
  ...
  return (
    ...
    >
      {subtitle && (
        <p className="text-xs sm:text-sm tracking-[0.3em] text-mas-red uppercase font-light mb-3">
          {subtitle}
        </p>
      )}
      {title && (
    ...
    >
      <Accordion type="single" collapsible className="w-full">
        {data.map((item) => (
          ...
          >
            <AccordionTrigger className="text-left text-sm sm:text-base font-semibold text-gray-700 py-7 hover:text-mas-red hover:no-underline transition-colors duration-200 [&>svg]:text-mas-red [&>svg]:shrink-0">
              {item.question}
            </AccordionTrigger>
